...
def test_start_command_and_service_selection(tmp_path, monkeypatch):
    monkeypatch.setenv("LAB_DIR", str(tmp_path))
    module = importlib.import_module("framework.lab_config")
    config = {"litellm-bin": "/bin/litellm", "litellm-config": "/tmp/config.yaml",
              "litellm-url": "http://localhost:4000", "bridge": "yes", "secretary": "off"}
    assert module.start_command(config) == "/bin/litellm --config /tmp/config.yaml --port 4000"

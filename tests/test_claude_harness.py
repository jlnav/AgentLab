...
def test_engineer_answer_records_query_and_session(fake_claude_sdk, monkeypatch, tmp_path, capsys):
    ...
    fake_claude_sdk.plan_client(turns=[fake_claude_sdk.turn(
...
def test_secretary_answer_includes_live_agent_status(fake_claude_sdk, monkeypatch, tmp_path):
    ...
    fake_claude_sdk.plan_client(turns=[fake_claude_sdk.turn(
        ...
        expected_query=lambda prompt: "Research agents running:\nagent1" in prompt,
    )])
    client = fake_claude_sdk.module.ClaudeSDKClient(options=object())
    ...
    assert (tmp_path / "run" / "secretary_session").read_text() == f"session-2\n{tmp_path}\n"
